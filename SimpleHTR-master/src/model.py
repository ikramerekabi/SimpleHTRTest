...
class Model:
    ...
    def setup_cnn(self) -> None:
        ...
        for i in range(num_layers):
            ...
            conv_norm = tf.compat.v1.keras.layers.BatchNormalization()(conv)
    ...
    def setup_rnn(self) -> None:
        ...
        num_hidden = 256
        cells = [tf.compat.v1.keras.layers.LSTMCell(units=num_hidden) for _ in
                 range(2)]  # 2 layers

        # stack basic cells
        stacked = tf.compat.v1.keras.layers.StackedRNNCells(cells)

        # bidirectional RNN
        # BxTxF -> BxTx2H
        # cell = tf.compat.v1.keras.layers.RNN(cell_fw=stacked, cell_bw=stacked, inputs=rnn_in3d,
        #                                                         dtype=rnn_in3d.dtype)

        #(fw, bw), _ = tf.compat.v1.keras.layers.Bidirectional(cell, merge_mode='concat')
